...
export const styles = {
  ...
  tabButton: (active) =>
    `px-4 py-2 rounded-lg text-sm font-medium capitalize ${active
  ...
  paymentBadge: (isOnline) =>
    `flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${isOnline
  ...
  // StockBadge
  stockBadge: (stock) => `px-2.5 py-1 text-xs font-medium rounded-full ${stock > 10
  ...
  sidebar: {
    container: (isCollapsed) =>
      `bg-gradient-to-t from-[#2B5876] to-[#43C6AC] text-white h-full p-4 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'}`,
    ...
    navItem: (isCollapsed, isActive) =>
      `group flex items-center px-4 py-3 rounded-lg transition-all duration-300 ${isActive
